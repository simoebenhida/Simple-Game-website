...
$requeteTextt = "SELECT * FROM validation WHERE refer ='" .$refer. "'";
$reqSqll = mysqli_query($co, $requeteTextt);
$tabt = lectureTableauPhpResultatRequete($reqSqll);
$codepaiement=$tabt['codepaiement'][0];
$utilisateur=$tabt['utilisateur'][0];
$mdp=$tabt['mdpp'][0];
?>
 <div class="container">
    	<div class="bg">
...
<div class="mod-md">
                        ...
                        <h2 class="title text-center">Finaliser Achats</h2>
	...
	{
		$u =0;
		  $requeteie = "SELECT * FROM validation WHERE refer ='" . $refer. "'";
		...
		if($u == 0){
		insertintovalidation($_POST['refer'],$_POST['codepaiement'],$_POST['utilisateur'],$_POST['mdp']);
		}
	
		
				
    
    if (!empty($_POST["codepaiement"])) {	
        
        	$reqConducteur5="UPDATE validation SET codepaiement='" .$_POST['codepaiement']. "' where refer='" .$refer. "'";
$sqlqueryconducteur = mysqli_query($co, $reqConducteur5);

    }
	 if (!empty($_POST["utilisateur"])) {	
        
        	$reqConducteur5="UPDATE validation SET utilisateur='" .$_POST['utilisateur']. "' where refer='" .$refer. "'";
			$sqlqueryconducteur = mysqli_query($co, $reqConducteur5);
    }
	if (!empty($_POST["mdp"])) {	
        
        	$reqConducteur5="UPDATE validation SET mdpp='" .$_POST['mdp']. "' where refer='" .$refer. "'";
$sqlqueryconducteur = mysqli_query($co, $reqConducteur5);
...
<p><strong><?php echo $name;?> </strong></p>
		   ...
		   <div class="col-sm-4 invoice-col">
	<p>A</p>
<p><strong> Mohamed Benhida </strong></p>
</br>
		</div>
		</div>
...
<div class="callout callout-info">
              ...
              <p> Vous allez recevoir votre commande dans 2h max si les informations sont vrais.</p>
              </div>

<form method='post' action="">
<dl><dt>RP:</dt><dd><input type="text" value="<?php if($cbrp == 11)
		{
			echo '1580';
		}
		else{
		...
		} ?> RP" readonly="true"  id="cus_email" class="txt w3010 mr10" name="cbrp"></dd></dl>
<dl><dt>Prix:</dt><dd><input type="text" value="<?php echo $prix;?> DH" readonly="true" class="txt w3010 mr10" id="cus_email" name="prix"></dd></dl>
       
<dl><dt>Code de commande:</dt><dd><input type="text" value="<?php echo $refer;?>" readonly="true" class="txt w3010 mr10" id="cus_email" name="refer"></dd></dl>
                           
						 

<dl><dt>Code de paiement:</dt><dd><input type="text" class="w300 txt mr10" name="codepaiement" value="<?php if($codepaiement != NULL)
						   {
						   echo $codepaiement;
						   }
							   ?>" required></dd></dl>
...
<dl><dt>Utilisateur LOL:</dt><dd><input type="text" class="w300 txt mr10" name="utilisateur" value="<?php if($utilisateur != NULL)
						   {
						   echo $utilisateur;}
							   ?>" required></dd></dl>
							   ...
							   <dl><dt>Mot de passe LOL:</dt><dd><input type="password" class="w300 txt mr10" name="mdp" value="<?php if($mdp != NULL)
						   {
						   echo $mdp;}
							   ?>" required></dd></dl>
							   
							    <dl><dt>Confirmer votre mot de passe:</dt><dd><input type="password" class="w300 txt mr10" name="mdp2" value="<?php if($mdp != NULL)
						   {
						   echo $mdp;}
							   ?>" required></dd></dl>


							<a href="commandes" ><button type="button" class="btn btn-primary pull-left">Annuler</button></a>
							<button type="submit" class="btn btn-primary pull-right" style="margin-right: 50px;">
             Valider
          </button>
		  </form>
